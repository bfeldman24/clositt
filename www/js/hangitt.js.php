...
if (typeof $ != "function" || 
    ...
    alert("Need to press the script 1 more times. Sorry");
    ...
    $(document).on("click","#hangitt-submit", function(){
        ...
        url += "?src=" + src.replace(/\?/g, '~~').replace(/\&/g, '~~~');
        
        if (link != null){
            url += "&link=" + link.replace(/\?/g, '~~').replace(/\&/g, '~~~');
        }
        
        url += "&page=" + location.href.replace(/\?/g, '~~').replace(/\&/g, '~~~');
    ...
    $(document).on("click","#hangitt-cancel", function(){
        $("#hangitt-dialog").remove();        
    });
    
    $(document).on("click","#close-itt", function(){
        $("#hangitt").remove();
        $("#hangitt-mask").remove();
    ...
    $(document).ready(function(){        
        ...
        $("body").append(
            ...
            .append(
                $("<img>").attr("src","http://www.clositt.com/css/images/logo.png").attr("alt", "Clositt.com").attr("style", "position: absolute !important; top: 15px !important; left: 20px !important;")
            ).append(
                ...
                $("<div>").addClass("hangitt-btn").attr("id","close-itt").attr("style","position: absolute; top: 15px; right: 20px; border: 1px solid rgb(204, 204, 204); border-radius: 5px; padding: 5px 10px; color: rgb(204, 204, 204); cursor: pointer;font-family: Open Sans;").text("Close")            
            )
        ).append(
            $("<div>").attr("id","hangitt-mask").attr("style","background: none repeat scroll 0 0 rgba(240, 240, 240, 0.8);height: 100%;left: 0;position: fixed;top: 0;width: 100%;z-index: 10000;")
        );
